...
let charToValue = Map.empty.Add('2',2).Add('3',3).Add('4',4)
                           .Add('5',5).Add('6',6).Add('7',7)
                           .Add('8',8).Add('9',9).Add('T',10)
                           .Add('J',11).Add('Q',12).Add('K',13)
                           .Add('A',14)

let getStraightValue (hand: string array) =
  let sortedHand = Array.sort [|for i in 0..4 -> charToValue.[hand.[i].[0]]|]
  let mutable index = 1
  let isStraight = not (Array.exists (fun e -> if index < sortedHand.Length && e + 1 <> sortedHand.[index] then true else index <- index + 1; false) sortedHand)
  if isStraight then sortedHand.[4] else -1

let isFlush (hand: string array) =
  not (Array.exists (fun (element: string) -> element.[1] <> hand.[4].[1]) hand)

let getMultiples (hand: string array) =
  let mutable multiples = Map.empty
  for e in hand do
    if multiples.ContainsKey(charToValue.[e.[0]]) then
      multiples <- multiples.Add(charToValue.[e.[0]], multiples.[charToValue.[e.[0]]] + 1)
    else
      multiples <- multiples.Add(charToValue.[e.[0]], 1)
  multiples

let getHandValue (hand: string array) =
  let flush = isFlush hand
  let straightValue = getStraightValue hand
  if flush && straightValue <> -1 then
    // Royal flush / Straight flush
    [9,straightValue]
  else
    let multiples = getMultiples hand
    if multiples.Count = 2 then
      let mutable fourOfAKind = 0
      let mutable kicker = 0
      let mutable topHouse = 0
      let mutable bottomHouse = 0
      for e in multiples do
        if e.Value = 4 then
          fourOfAKind <- e.Key
        else if e.Value = 3 then
          bottomHouse <- e.Key
        else if e.Value = 2 then
          topHouse <- e.Key
        else if e.Value = 1 then
          kicker <- e.Key
      if fourOfAKind = 0 then
        // Full house
        [7, bottomHouse * 100 + topHouse]
      else
        // Four of a kind
        [8,fourOfAKind * 100 + kicker]
    else if flush then
      // Flush
      let sortedHand = Array.sort [|for i in 0..4 -> charToValue.[hand.[i].[0]]|]
      let mutable returnValue = 0
      for i in 0 .. sortedHand.Length - 1 do
        returnValue <- returnValue + sortedHand.[i] * pown 10 (2 * i)
      [6, returnValue]
    else if straightValue <> -1 then
      // Straight
      [5, straightValue]
    else if multiples.Count = 3 then
      let mutable twoPair = []
      let mutable kickers = []
      let mutable threeOfAKind = 0
      for e in multiples do
        if e.Value = 2 then
          twoPair <- e.Key :: twoPair
        else if e.Value = 3 then  
          threeOfAKind <- e.Key
        else
          kickers <- e.Key :: kickers
      if threeOfAKind = 0 then
        // Two pair
        twoPair <- List.sort twoPair
        [3, twoPair.[1] * 10000 + twoPair.[0] * 100 + kickers.[0]]
      else
        // Three of a kind
        kickers <- List.sort kickers
        [4, threeOfAKind * 10000 + kickers.[1] * 100 + kickers.[0]]
    else if multiples.Count = 4 then
      let mutable kickers = []
      let mutable pair = 0
      for e in multiples do
        if e.Value = 1 then 
      ...
      kickers <- List.sort kickers
      // One pair
      [2,pair * pown 10 6 + kickers.[2] * pown 10 4 + kickers.[1] * pown 10 2 + kickers.[0]]
    else
      // Highest card
      let sortedHand = Array.sort [|for i in 0..4 -> charToValue.[hand.[i].[0]]|]
      let mutable total = 0
      for i in sortedHand.Length-1.. -1 ..0 do
        total <- total + sortedHand.[i] * pown 10 (2 * i)
      [1, total]

let mutable player1Wins = 0
for e in pokerHands do
  let handValue1 = getHandValue e.[0]
  let handValue2 = getHandValue e.[1]
  if handValue1.[0] > handValue2.[0] || (handValue1.[0] = handValue2.[0] && handValue1.[1] > handValue2.[1]) then
    player1Wins <- player1Wins + 1

printfn "%A" player1Wins
